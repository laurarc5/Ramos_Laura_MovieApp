...
var MainURL="https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=4e1eb07fd91f15362c5d430ba0319d2b"; 
    ...
    theList.empty();
     var request = $.ajax({
        ...
        request.done(function( moviesList ) {
            ...
            theList.append(
            '<li><a Onclick="javascript:isFav('+moviesList.results[i].id+',\'webDetail\')">'+
...
function deleteFav(id){
    db.executeSql('SELECT * FROM favorites WHERE id=?', [id], function(res) {
        ...
        var backdrop = res.rows.item(0).backdrop_path;
                ...
                $('#fav').attr("onclick",'addFav('+id+',"'+title+'","'+vote+'","'+date+'","'+genres+'","'+overview+'","'+poster+'","'+backdrop+'")');
                selectDDBB();
              }, function(error) {
...
function addFav(id,title,vote,date,genres,overview,poster,backdrop){//pone favorito
...
function MovieDetail(id,isfav){


     var request = $.ajax({
          url: "https://api.themoviedb.org/3/movie/"+id+"?api_key=4e1eb07fd91f15362c5d430ba0319d2b",
          method: "GET",
        });
        ...
        request.done(function( result ) {
            ...
            for(ig=0;ig<result.genres.length;ig++){
              ...
              if(isfav == 1){
                  ...
                  starLink='onclick=\'addFav("'+result.id+'","'+correctTitleString+'","'+result.vote_average+'","'+result.release_date+'","'+GenreString+'","'+correctOverviewString+'","'+result.poster_path+'","'+result.backdrop_path+'")\'';
